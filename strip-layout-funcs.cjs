...
content = content.replace(/function earnStampProgress[\s\S]*?playSound\('success', isMuted\);\n  }/, '');
content = content.replace(/function claimStampMiles[\s\S]*?playSound\('success', isMuted\);\n  }/, '');

content = content.replace(/async function handleSavePassport[\s\S]*?isEditingPassport = false;\n  }/, `async function handleSavePassport(updated: Passport) {
...
content = content.replace(/async function handleRefuel[\s\S]*?isRefueling = false;\n    }\n  }/, '');

// 2. Remove giant fetchState
content = content.replace(/async function fetchState[\s\S]*?setTimeout\(\(\) => isSyncing = false, 500\);\n      }\n    }\n  }/, '');
...
content = content.replace(/bind:showPassportDrawer/g, 'bind:showPassportDrawer={dalStore.showPassportDrawer}');
