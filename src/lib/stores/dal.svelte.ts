...
export class DalState {
  ...
  async earnStampProgress(field: 'hasBoarded' | 'hasHosted' | 'hasChatted' | 'hasCustomized' | 'hasRequested') {
    ...
    if (typeof window !== 'undefined') {
      ...
      try {
        ...
        await fetch('/wp-json/dodo-air/v1/profiles/' + encodeURIComponent(this.passport.friendCode), {
          method: 'POST',
          headers,
      ...
      if (res.ok) {
        ...
        if (data.myPassport && (!this.passport.hasCreated || data.myPassport.hasCreated)) {
          ...
          if (JSON.stringify(this.passport) !== JSON.stringify(updatedPassport)) {
            ...
            if (typeof window !== 'undefined') localStorage.setItem('dal_passport', JSON.stringify(this.passport));
          }
        }
      }
    } catch (err) {
