...
{#if isOpen}
  ...
  <div class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
    <div class="absolute inset-0" onclick={() => { playSound('beep', isMuted); onClose(); }}></div>
    
    <div class="relative w-full max-w-4xl bg-white border-4 border-[#E6DFC7] rounded-[32px] p-5 shadow-2xl flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-dashed border-[#E6DFC7] pb-3 text-left pr-10">
        <div>
          <h2 class="font-system font-black text-sm text-[#0084CC] tracking-wide flex items-center gap-1.5 uppercase font-bold">
            <Radio class="w-4 h-4 animate-pulse shrink-0" /> DAL Traffic Control & Radar Center
          </h2>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5 text-left">
        <!-- Stat 1: Page Views -->
        <div class="bg-rose-50/40 border-2 border-rose-100 rounded-2xl p-3.5 shadow-xs relative overflow-hidden">
        ...
        <div class="bg-violet-50/40 border-2 border-violet-100 rounded-2xl p-3.5 shadow-xs relative overflow-hidden">
          <div class="absolute right-2 top-2 opacity-5 pointer-events-none">
        ...
        >
          ...
          <div class="flex items-baseline gap-1">
            <span class="font-mono font-black text-lg sm:text-xl text-emerald-950 leading-none font-bold">
          ...
          <p class="text-[9.5px] text-slate-500 leading-snug mt-1 font-sans font-semibold">
            {dalStore.systemMode === 'DAL' ? 'Active flight plans open now vs all-time.' : 'Active dreams open now vs all-time.'}
          </p>
        </div>
        ...
        >
          ...
          <div class="flex items-baseline gap-1">
            <span class="font-mono font-black text-lg sm:text-xl text-amber-950 leading-none font-bold">
              {totalPassengers}<span class="text-slate-400 font-medium text-xs">/{alltimePassengers}</span>
            </span>
          ...
          <p class="text-[9.5px] text-slate-500 leading-snug mt-1 font-sans font-semibold">
            {dalStore.systemMode === 'DAL' ? 'Travelers currently checked in vs all-time.' : 'Travelers currently dreaming vs all-time.'}
          </p>
        </div>
