<script lang="ts">
	...
	let myFlight = $derived(
		dalStore.flights.find(
			(f) =>
				f.status !== 'Closed' &&
				f.hostName.toLowerCase() === dalStore.passport.villagerName.toLowerCase() &&
				f.islandName.toLowerCase() === dalStore.passport.islandName.toLowerCase()
		)
	);
...
<Box class="max-w-4xl mx-auto w-full">
	{#if !myFlight}
		<Box
			class="bg-white rounded-[36px] border-4 border-[#0084CC]/10 shadow-[0_8px_0_0_rgba(0,132,204,0.05)] p-6 text-center space-y-6"
		>
			<Box class="max-w-xs mx-auto">
				<Box
					class="w-20 h-20 bg-[#F0F9FF] border-2 border-[#0084CC] rounded-full flex items-center justify-center text-5xl mx-auto shadow relative transform -rotate-12"
				>
					🛩️
					<Box
						class="absolute -bottom-1 -right-1 bg-[#FFCC00] text-[#006094] text-xs font-system font-black px-1.5 py-0.5 rounded-full border border-white uppercase"
					>
						DAL-X
					</Box>
				</Box>
			...
			<Box>
				<Text tag="h2" class="text-xl font-black text-[#0084CC]"
					>Your Private DAL Seaplane Hangar</Text
				>
				<Text
					tag="p"
					class="text-xs text-slate-400 font-system mt-1 uppercase tracking-wider font-bold"
				>
			...
			<AcnhBubble
				...
				dialogText={dalStore.systemMode === 'DAL'
					? DIALOGS.hubTab.active
					: DIALOGS.hubTab.lunaActive}
	...
	{:else}
		<Box class="space-y-5">
			<Box
				class="bg-[#006094] text-white rounded-[32px] p-5 shadow border-b-4 border-[#FFCC00] relative overflow-hidden"
			>
				<Box class="absolute right-0 top-0 opacity-10 text-9xl pointer-events-none select-none"
					>🛩️</Box
				>
				<Box class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
					<Box>
						<Text
							tag="span"
							class="bg-[#FFCC00] text-[#006094] text-xs font-black tracking-widest px-2.5 py-0.5 rounded-full font-system uppercase"
						>
							ACTIVE PILOT CONSOLE
						</Text>
						<Text tag="h2" class="text-2xl font-black font-system tracking-normal mt-1">
							Flight {myFlight.flightNumber || myFlight.id} to '{myFlight.islandName}'
						</Text>
						<Text tag="p" class="text-xs text-sky-200 mt-0.5">
							Island host: <Text tag="strong">{myFlight.hostName}</Text> | {dalStore.systemMode ===
							'DAL'
								? 'Gate'
								: 'Dream'} Theme: <Text tag="strong"
								>{(dalStore.systemMode === 'DAL'
									? GATE_THEMES[myFlight.gate]
									: DREAM_THEMES[myFlight.gate]
								)?.name}</Text
							>
						</Text>
					</Box>
					<Box class="bg-black/20 border border-white/10 p-3 rounded-2xl text-center font-system">
						<Text tag="span" class="block text-xs text-sky-200 font-bold uppercase"
							>My Dodo Code</Text
						>
						<Text
							tag="span"
							class="text-2xl font-black text-[#FFCC00] tracking-widest leading-none mt-0.5 block uppercase"
						>
							{myFlight.dodoCode}
						</Text>
					</Box>
			...
			<Box class="grid grid-cols-1 md:grid-cols-12 gap-5">
				<Box class="md:col-span-7 space-y-4">
					<Box class="bg-white rounded-[32px] p-5 border-2 border-[#0084CC]/10 shadow-sm space-y-4">
						<Text
							tag="h3"
							class="font-system font-black text-xs text-[#0084CC] uppercase tracking-wide border-b border-slate-100 pb-2"
						>
					...
					<Box class="bg-white rounded-[32px] p-5 border-2 border-[#0084CC]/10 shadow-sm">
						<Box class="flex items-center justify-between border-b border-slate-100 pb-2 mb-3">
							<Text
								tag="h3"
								class="font-system font-black text-xs text-[#0084CC] uppercase tracking-wide"
							>
						...
						{#if myFlight.passengers.length === 0}
							<Box class="py-8 text-center font-system text-slate-400 text-xs">
								{dalStore.systemMode === 'DAL'
									...
									: `No dreamers have arrived at Dream ${myFlight.gate} yet. The bed is empty!`}
							</Box>
						{:else}
							<Box class="space-y-2">
								{#each myFlight.passengers as p}
									...
									>
										<Box class="flex items-center gap-2">
											...
											<Box class="text-xs">
												<Text tag="span" class="font-system font-black text-slate-700"
													>{p.name}</Text
												>
												<Text tag="span" class="text-xs font-system text-slate-400 block"
													>from {p.island}</Text
												>
											</Box>
										</Box>
					...
					<Box class="bg-white rounded-[32px] p-5 border-2 border-[#0084CC]/10 shadow-sm space-y-4">
						...
						<Box
							class="bg-[#FAF8F2] p-3 rounded-2xl border border-[#E6DFC7]/50 text-xs text-slate-600 font-system italic leading-relaxed"
						>
						...
						{#if myFlight.review}
							...
							>
								<Text
									tag="h4"
									class="font-system font-black text-[#D35400] text-xs flex items-center gap-1"
								>
						...
						{:else}
							...
							>
								<Text tag="p" class="text-sm text-slate-500 mb-2 leading-relaxed">
									Ask Orville to compile Wilbur's flight deck observations and publish an official
									Travel Review brochure!
								</Text>
								<Button
					...
					>
						<Box class="flex items-center gap-2 border-b border-[#FFEAA7] pb-2 mb-3">
							...
							<Box>
								<Text
									tag="h3"
									class="font-system font-black text-xs text-[#0084CC] uppercase leading-none"
									>Smart Flight Matchmaker</Text
								>
								<Text
									tag="span"
									class="text-xs font-system text-slate-400 font-bold uppercase mt-0.5 block"
									>ORVILLE'S MATCH COUNTER</Text
								>
							</Box>
						</Box>
						...
						{#if matches.length === 0}
							<Box class="text-center py-8 text-slate-400 font-system text-xs space-y-2">
								...
								<Text tag="p" class="text-sm leading-relaxed"
									>{dalStore.systemMode === 'DAL'
										? `No standby flyers are looking for Gate Category ${myFlight.gate} at the moment. Keep radar active!`
										: `No standby dreamers are looking for Dream Category ${myFlight.gate} at the moment.`}</Text
								>
							</Box>
						{:else}
							<Box class="space-y-3">
								<Box
									class="bg-white/80 p-2.5 rounded-2xl border border-[#FFEAA7] text-xs text-[#4A4A4A]"
								>
									💡 <Text tag="strong">Orville:</Text> "Look! We have standby passengers looking to
									match your flight's category! Clear them for immediate boarding!"
								</Box>
								{#each matches as match}
									<Box
										class="bg-white p-3 rounded-2xl border-2 border-[#0084CC]/20 shadow-xs space-y-2"
									>
										<Box class="flex items-start gap-2">
											<Text tag="span" class="text-xl">{match.avatar}</Text>
											<Box>
												<Text tag="span" class="font-system font-black text-xs block text-slate-700"
													>{match.name}</Text
												>
												<Text tag="span" class="text-xs text-slate-400 font-system leading-none"
													>from {match.island}</Text
												>
											</Box>
										</Box>
