<script lang="ts">
	...
	let passengerIndex = $derived(
		passengersList.findIndex((p: Passenger) =>
			p.userId
	...
	let fromCode = $derived(
		(passport.hasCreated ? passport.islandName : 'HOME')
			.replace(/[^a-zA-Z]/g, '')
			.slice(0, 3)
			.toUpperCase()
	);
...
{#if selectedFlight}
	...
	<div
		class="fixed inset-0 bg-[#006094]/40 backdrop-blur-md flex flex-col items-center justify-center p-4 z-50 gap-6 overflow-y-auto"
		transition:fade={{ duration: 200 }}
	>
		<!-- Dialogue Bubble -->
		<div
			class="w-full max-w-4xl shrink-0 mt-8 md:mt-0"
		...
		>
			<AcnhBubble
				...
				dialogText={isPassengerCheckedIn
					? dalStore.systemMode === 'DAL'
						? DIALOGS.boardingPassModal.dalCheckedIn(flightNumberDisplay)
						: DIALOGS.boardingPassModal.lunaCheckedIn(flightNumberDisplay)
					: passengersList.length >= capacity
						? dalStore.systemMode === 'DAL'
							? DIALOGS.boardingPassModal.dalFull(capacity)
							: DIALOGS.boardingPassModal.lunaFull(capacity)
						: dalStore.systemMode === 'DAL'
							? DIALOGS.boardingPassModal.dalBook(flightNumberDisplay)
							: DIALOGS.boardingPassModal.lunaBook(flightNumberDisplay)}
			></AcnhBubble>
		</div>
		...
		>
			...
			<div class="bp-header {dalStore.systemMode === 'DAL' ? 'bp-header--dal' : 'bp-header--luna'}">
				...
				<div class="bp-header-meta">
					<div class="bp-header-top-row">
						<span class="bp-header-flight">{flightNumberDisplay}</span>
						<span class="bp-header-status bp-header-status--{selectedFlight.status.toLowerCase()}"
							>{selectedFlight.status.toUpperCase()}</span
						>
					</div>
					<span class="bp-header-label"
						>{dalStore.systemMode === 'DAL' ? 'BOARDING PASS' : 'DREAM TICKET'}</span
					>
				</div>
				<button
					onclick={() => {
					...
					class="bp-close-btn {dalStore.systemMode === 'DAL'
						? 'bp-close-btn--dal'
						: 'bp-close-btn--luna'}"
					aria-label="Close"
				>
			...
			<div class="bp-body">
				...
				<div class="bp-airport-hero">
					<div class="bp-airport">
						<span
							class="bp-airport-code {dalStore.systemMode === 'DAL'
								? 'text-[#0084CC]'
								: 'text-[#4B0082]'}"
						>
							{fromCode}
						...
						<span class="bp-airport-label"
							>{dalStore.systemMode === 'DAL' ? 'DEPARTURE' : 'WAKING ISLAND'}</span
						>
					</div>

					<div class="bp-route-arrow">
						<div
							class="bp-route-line {dalStore.systemMode === 'DAL'
								? 'bg-[#0084CC]'
								: 'bg-[#4B0082]'}"
						></div>
					...
					<div class="bp-airport bp-airport--right">
						...
						<span class="bp-airport-label"
							>{dalStore.systemMode === 'DAL' ? 'DESTINATION' : 'DREAM ISLAND'}</span
						>
					</div>
				</div>
				...
				<div class="bp-data-grid bp-data-grid--simplified">
					<div class="bp-data-cell">
						...
						<span class="bp-field-value font-mono"
							>{(passport.hasCreated
								? passport.villagerName
								: 'GUEST PASSENGER'
							).toUpperCase()}</span
						>
					</div>
					...
					<div class="bp-data-cell bp-data-cell--plane">
						<span class="bp-field-label"
							>{dalStore.systemMode === 'DAL' ? 'AIRCRAFT' : 'VESSEL'}</span
						>
				...
				<div class="bp-action-zone">
					...
					{:else if passengersList.length >= capacity}
						<div class="bp-full-notice">
							...
							<span class="bp-full-label"
								>{dalStore.systemMode === 'DAL' ? 'SEAPLANE FULL' : 'DREAM BED FULL'}</span
							>
							<button
								type="button"
					...
					{:else}
						<div class="w-full space-y-1.5">
							...
							<button
								...
								disabled={selectedFlight.status === 'Closed' ||
									selectedFlight.status === 'Departed'}
								class="bp-board-btn {dalStore.systemMode === 'DAL'
									? 'bp-board-btn--dal'
									: 'bp-board-btn--luna'}"
							>
			...
			<div class="bp-tear-line">
				...
				<div
					class="bp-tear-dashes {dalStore.systemMode === 'DAL'
						? 'border-[#0084CC]/30'
						: 'border-[#4B0082]/30'}"
				></div>
			...
			<div class="bp-stub">
				...
				<div class="bp-barcode">
					{#if qrCodeUrl}
						<img src={qrCodeUrl} alt="Boarding Pass QR Code" class="w-[150px] h-[150px]" />
					{/if}
				</div>
...
<style>
	...
	.bp-header--dal {
		background: linear-gradient(135deg, #004a7c 0%, #0084cc 60%, #00a8e8 100%);
	}
	.bp-header--luna {
		background: linear-gradient(135deg, #1a0033 0%, #4b0082 60%, #7b52ab 100%);
	}
	...
	.bp-header-status--scheduled {
		background: rgba(34, 197, 94, 0.25);
		border-color: rgba(34, 197, 94, 0.5);
		color: #86efac;
	}
	.bp-header-status--open {
		background: rgba(34, 197, 94, 0.25);
		border-color: rgba(34, 197, 94, 0.5);
		color: #86efac;
	}
	.bp-header-status--closed {
		background: rgba(239, 68, 68, 0.25);
		border-color: rgba(239, 68, 68, 0.5);
		color: #fca5a5;
	}
	.bp-header-status--departed {
	...
	.bp-board-btn--dal:hover {
		background: #ffd740;
	}
	.bp-board-btn--luna:hover {
		background: #e8b5e8;
	}
