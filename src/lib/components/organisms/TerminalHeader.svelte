...
<Box
	tag="header"
	class="w-full mb-6 {dalStore.systemMode === 'DAL'
		? 'bg-[#0084CC] border-[#FFCC00] shadow-[0_8px_32px_rgba(0,132,204,0.15)]'
		: 'bg-[#4B0082] border-[#DDA0DD] shadow-[0_8px_32px_rgba(75,0,130,0.15)]'} text-white rounded-[24px] p-3.5 lg:p-4 border-b-4 flex flex-col lg:flex-row items-center justify-between gap-4 relative z-50 transition-all duration-500"
>
	<!-- Subtle inner background glow -->
	<Box
		class="absolute inset-0 rounded-[20px] opacity-15 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] {dalStore.systemMode ===
		'DAL'
			...
			<Box class="flex items-center gap-2">
				<Text
					...
					class="{dalStore.systemMode === 'DAL'
						...
						: 'bg-[#DDA0DD] text-[#4B0082]'} text-[10px] font-black tracking-widest px-2.5 py-0.5 rounded-full font-system shadow-sm transition-colors duration-500"
				>
				...
				{#if dalStore.isSyncing}
					<Text
						tag="span"
						class="flex items-center gap-1 text-[11px] {dalStore.systemMode === 'DAL'
							? 'text-sky-200'
							: 'text-purple-200'} animate-pulse font-system"
					>
						<RefreshCw class="w-2.5 h-2.5 animate-spin" />
						{dalStore.systemMode === 'DAL' ? 'Radar Active' : 'Dreaming...'}
					</Text>
				{/if}
			</Box>
			<h1
				class="text-xl lg:text-2xl font-black tracking-normal text-white drop-shadow-sm mt-0.5 leading-none"
			>
				{#if dalStore.systemMode === 'DAL'}
					<span class="text-[#FFCC00]">Online Terminal</span>
				{:else}
					<span class="text-[#DDA0DD]">Online Terminal</span>
				{/if}
			</h1>
			<Box
				class="flex items-center gap-3 mt-1.5 text-[10px] font-medium text-white font-system uppercase tracking-wider opacity-90 drop-shadow-sm"
			>
				...
				<Box class="flex items-center gap-1.5">
					<Ticket
						class="w-3.5 h-3.5 {dalStore.systemMode === 'DAL'
							? 'text-[#FFCC00]'
							: 'text-[#DDA0DD]'}"
					/>
					<span>FF Miles {dalStore.passport.miles?.toLocaleString() || 0}</span>
				</Box>
			</Box>
		</Box>
	</Box>
	...
	<Box class="flex items-center justify-center z-10 w-full lg:w-1/3 min-h-[36px]">
		{#if dalStore.systemMode === 'DAL'}
			<img
				src={logoUrl}
				alt="Dodo Airlines"
				class="h-8 lg:h-[125px] object-contain drop-shadow-[0_2px_4px_rgba(0,0,0,0.15)]"
			/>
		{/if}
	</Box>
	...
	<Box
		class="flex flex-col sm:flex-row flex-wrap items-center gap-3 z-10 w-full lg:w-1/3 sm:justify-end lg:justify-end"
	>
		<!-- Combined Status Panel: Date + Clock -->
		<Box
			class="flex items-center gap-3 {dalStore.systemMode === 'DAL'
				? 'bg-[#006094]/80'
				: 'bg-[#290048]/80'} rounded-2xl p-2.5 px-4 border border-white/10 shadow-inner font-system text-sm transition-colors duration-500 w-full sm:w-auto justify-center sm:justify-start"
		>
			<!-- Date -->
			<Box class="flex flex-col leading-tight">
				<Box class="flex items-center gap-1.5">
					...
					<Text
						...
						class="{dalStore.systemMode === 'DAL'
							? 'text-sky-200'
							: 'text-purple-200'} uppercase text-[9px] font-black tracking-wider leading-none"
						>DATE</Text
					>
				</Box>
				<Text tag="span" class="text-white font-bold text-xs whitespace-nowrap mt-0.5 pl-5"
					>{formattedDay} {formattedDate}</Text
				>
			</Box>

			<Box
				class="h-6 w-px {dalStore.systemMode === 'DAL' ? 'bg-sky-500/20' : 'bg-purple-500/20'}"
			/>
			...
			<Box class="flex flex-col leading-tight">
				...
				<Text
					tag="span"
					class="text-white font-bold tracking-wider text-xs whitespace-nowrap mt-0.5 pl-5"
				>
					{formattedTime}
					{#if timeZone}
