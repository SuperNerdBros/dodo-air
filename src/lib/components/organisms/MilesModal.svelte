          ...
          <Box class={`p-3.5 rounded-3xl border-2 relative overflow-hidden flex items-start gap-3 transition-all ${
            ...
            <Box class="relative flex-shrink-0">
              ...
              {#if isClaimed}
                <Box class="absolute -inset-1 rounded-full bg-red-600/10 border-2 border-red-600 flex items-center justify-center transform rotate-12 select-none pointer-events-none scale-105">
                  <Text tag="span" class="text-xs font-system font-black text-red-600 tracking-normal uppercase leading-none">OK!</Text>
                </Box>
              {/if}
            </Box>
            ...
            <Box class="flex-1 min-w-0">
              <Box class="flex items-center justify-between gap-1">
                ...
                <Text tag="span" class="font-system text-xs text-[#FF9F43] font-black shrink-0">+{challenge.miles}</Text>
              </Box>
              <Text tag="p" class="text-sm text-slate-400 mt-1 leading-normal font-sans">{challenge.desc}</Text>
              
              <Box class="mt-2.5">
                {#if isClaimed}
                  <Text tag="span" class="text-xs font-system font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full border border-red-100">
                    🏆 STAMPED & CLAIMED
                  </Text>
                {:else if isAchieved}
