<script lang="ts">
	...
	let {
		...
		isActive = true,
		isIntro = false,
		class: className = 'mx-auto w-full lg:w-3/4 max-w-none lg:max-w-5xl',
...
{#if isActive}
	<div
		class="pointer-events-auto dialogue {className}"
		style="--dialogue-font: {title ? 'FOT-RodinBokutoh Pro EB' : 'FOT-Seurat Pro B'};"
	>
				...
				{#if dialogText}
					<div class="flex gap-4 items-start relative z-10">
						...
						{#if characterKey === 'Orville' || characterKey === 'Wilbur'}
							<div
								class="hidden sm:flex shrink-0 w-16 h-16 bg-[#FFFCEF] border-[3px] border-[#D1BFAe] rounded-full items-center justify-center text-4xl shadow-inner transform -rotate-6"
							>
								🦤
							</div>
						{:else if characterKey === 'Luna'}
							<div
								class="hidden sm:flex shrink-0 w-16 h-16 bg-[#F4EEFF] border-[3px] border-[#DDA0DD] rounded-full items-center justify-center text-4xl shadow-inner transform -rotate-6"
							>
								💤
							</div>
						{/if}
						...
						<div class="flex-1 py-1">
							<p
								class="text-xl sm:text-2xl {characterKey === 'Luna'
									? 'text-[#4B0082]'
									: 'text-[#807256]'} leading-snug font-medium min-h-[3.6rem]"
							>
								{typedText}
								{#if !textDone}
									<span class="inline-block w-1.5 h-5 bg-current animate-pulse ml-0.5 align-middle"
									></span>
								{/if}
							</p>
							{@render children?.()}
