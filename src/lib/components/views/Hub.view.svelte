<script lang="ts">
	...
	let activeGateTheme = $derived(
		dalStore.systemMode === 'DAL'
			? GATE_THEMES[formGate] || GATE_THEMES[1]
			: DREAM_THEMES[formGate] || DREAM_THEMES[1]
	);
...
<div class="w-100 mx-auto pt-5">
	...
	{#if !myFlight}
			...
			<div class="mt-4 flex justify-center">
				<button
					onclick={() => {
						...
						dalStore.hubWizardInitialStep = 1;
						dalStore.showHubModal = true;
					}}
					class="w-full max-w-sm {dalStore.systemMode === 'DAL'
						? 'bg-[#FFCC00] hover:bg-[#FFD11A] text-[#006094] border-[#CC9900]'
						: 'bg-[#DDA0DD] hover:bg-[#e8b5e8] text-[#4B0082] border-[#ba80ba]'} font-system font-black py-3 rounded-2xl border-b-4 shadow transition-all uppercase tracking-wide text-xs cursor-pointer active:translate-y-1 active:border-b-0"
				>
		...
		<div
			class="mt-5 bg-white rounded-[36px] border-4 border-[#0084CC]/10 shadow-[0_8px_0_0_rgba(0,132,204,0.05)] p-6 text-center space-y-6 text-left"
		>
			<!-- Hangar Illustrative Header -->
			<div class="max-w-xs mx-auto">
				<div
					class="w-20 h-20 {dalStore.systemMode === 'DAL'
						? 'bg-[#F0F9FF] border-[#0084CC]'
						: 'bg-purple-50 border-[#4B0082]'} border-2 rounded-full flex items-center justify-center text-5xl mx-auto shadow relative transform -rotate-12 transition-colors duration-500"
				>
					{dalStore.systemMode === 'DAL' ? '🛩️' : '🛌'}
					<div
						class="absolute -bottom-1 -right-1 {dalStore.systemMode === 'DAL'
							? 'bg-[#FFCC00] text-[#006094]'
							: 'bg-[#DDA0DD] text-[#4B0082]'} text-xs font-system font-black px-1.5 py-0.5 rounded-full border border-white uppercase transition-colors duration-500"
					>
			...
			<div>
				<h2
					class="text-xl font-black {dalStore.systemMode === 'DAL'
						? 'text-[#0084CC]'
						: 'text-[#4B0082]'} text-center font-bold"
				>
					{dalStore.systemMode === 'DAL'
						? 'Your Private DAL Seaplane Hangar'
						: 'Your Personal Dream State'}
				</h2>
				<p
					class="text-xs text-slate-400 font-system mt-1 uppercase tracking-wider font-bold text-center"
				>
					{dalStore.systemMode === 'DAL'
						? 'PARKED & FUELED - READY TO WELCOME VISITORS'
						: 'BED PREPARED - READY TO SHARE YOUR DREAM'}
				</p>

				<!-- XP Progression Display -->
				<div class="flex items-center justify-center gap-4 mt-4">
					<div
						class="bg-slate-50 border border-slate-200 rounded-2xl px-6 py-2.5 text-slate-600 font-system shadow-sm flex items-center gap-3"
					>
						<div
							class="bg-amber-100 text-amber-500 w-8 h-8 rounded-full flex items-center justify-center shadow-inner text-sm"
						>
							<Sparkles class="w-4 h-4 fill-current" />
						</div>
						<div class="text-left leading-tight">
							<span class="block text-[9px] text-slate-400 uppercase font-black tracking-wider"
								>LIFETIME XP</span
							>
							<span class="font-bold text-sm text-[#0084CC]"
								>{dalStore.passport.xp?.toLocaleString() || 0}
								<span class="text-[10px] text-slate-400">XP</span></span
							>
						</div>
					</div>
				</div>
		...
		{#if !myFlight.dodoCode && isActive}
			...
			>
				<div class="flex gap-2 max-w-sm mt-3">
					...
					>
						{dalStore.systemMode === 'DAL' ? 'Submit' : 'Slumber'}
					</button>
				</div>
			</AcnhBubble>
		{/if}
		...
		<div class="space-y-5 text-left mt-5">
			...
			<div
				class="{dalStore.systemMode === 'DAL'
					? 'bg-[#006094] border-[#FFCC00]'
					: 'bg-[#4B0082] border-[#DDA0DD]'} text-white rounded-[32px] p-5 shadow border-b-4 relative overflow-hidden transition-colors duration-500"
			>
				...
				<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
					<div>
						<span
							class="{dalStore.systemMode === 'DAL'
								? 'bg-[#FFCC00] text-[#006094]'
								: 'bg-[#DDA0DD] text-[#4B0082]'} text-xs font-black tracking-widest px-2.5 py-0.5 rounded-full font-system uppercase font-bold transition-colors"
						>
						...
						<h2 class="text-2xl font-black font-system tracking-normal mt-1 font-bold">
							{dalStore.systemMode === 'DAL' ? 'Flight' : 'Dream'}
							{myFlight.flightNumber || myFlight.id}
							{dalStore.systemMode === 'DAL' ? 'to' : 'at'}
							{myFlight.islandName}
						</h2>
						<p
							class="text-xs {dalStore.systemMode === 'DAL'
								? 'text-sky-200'
								: 'text-purple-200'} mt-0.5"
						>
							Island host: <strong>{myFlight.hostName}</strong> | {dalStore.systemMode === 'DAL'
								? 'Gate'
								: 'Dream'} Theme:
							<strong
								>{(dalStore.systemMode === 'DAL'
									? GATE_THEMES[myFlight.gate] || GATE_THEMES[1]
									: DREAM_THEMES[myFlight.gate] || DREAM_THEMES[1]
								).name}</strong
							>
						</p>
					</div>
					...
					<div class="bg-black/20 border border-white/10 p-3 rounded-2xl text-center font-system">
						<span
							class="block text-xs {dalStore.systemMode === 'DAL'
								? 'text-sky-200'
								: 'text-purple-200'} font-bold uppercase"
						>
						...
						<span
							class="text-2xl font-black {dalStore.systemMode === 'DAL'
								? 'text-[#FFCC00]'
								: 'text-[#DDA0DD]'} tracking-widest leading-none mt-0.5 block uppercase font-bold"
						>
			...
			<div class="grid grid-cols-1 md:grid-cols-12 gap-5">
				...
				<div class="md:col-span-7 space-y-4">
					...
					<div
						class="bg-white rounded-[32px] p-5 border-2 {dalStore.systemMode === 'DAL'
							? 'border-[#0084CC]/10'
							: 'border-[#4B0082]/10'} shadow-sm space-y-4 transition-colors"
					>
						<h3
							class="font-system font-black text-xs {dalStore.systemMode === 'DAL'
								? 'text-[#0084CC]'
								: 'text-[#4B0082]'} uppercase tracking-wide border-b border-slate-100 pb-2 font-bold transition-colors"
						>
							{dalStore.systemMode === 'DAL'
								? '🚦 Seaplane Runway Controllers'
								: '🛌 Dream State Controllers'}
						</h3>
								...
								<button
									onclick={() => {
										...
										if (!myFlight.dodoCode && (status === 'Boarding' || status === 'Departed')) {
											alert(
												dalStore.systemMode === 'DAL'
													? 'Please provide a Dodo Code first!'
													: 'Please provide a Doze Code first!'
											);
											return;
										}
										handleUpdateStatus(myFlight.id, status);
									}}
									class="py-2 rounded-xl font-system font-black border transition-all text-sm cursor-pointer font-bold {isActive
					...
					{#if myFlight.status === 'Scheduled'}
						<div class="mt-4">
							<div
								class="bg-[#FFFCEF] rounded-[32px] border-4 border-[#FFEAA7] p-5 shadow-sm text-left"
							>
								<div class="flex items-start gap-4">
									<div
										class="w-12 h-12 bg-white rounded-full flex items-center justify-center text-2xl shadow-inner border-2 border-[#D1BFAe] shrink-0"
									>
										{dalStore.systemMode === 'DAL' ? '🦤' : '🔮'}
									</div>
									<div>
										<h3
											class="font-system font-black text-sm {dalStore.systemMode === 'DAL'
												? 'text-[#0084CC]'
												: 'text-[#4B0082]'} uppercase tracking-wide"
										>
											{dalStore.systemMode === 'DAL'
												? 'Flight Schedule Active'
												: 'Dream Schedule Active'}
										</h3>
										<p class="text-xs text-slate-600 mt-1 mb-3 font-medium leading-relaxed">
											{#if dalStore.systemMode === 'DAL'}
												"Roger that! You have {dalStore.mySchedules.length} routine flight times registered.
												Keep your Dodo Code blank when offline, and we'll automatically notify standby
												passengers during these hours!"
											{:else}
												"Your dream is scheduled... You have {dalStore.mySchedules.length} slumber times
												registered. Others will be drawn to your island during these peaceful moments."
											{/if}
										</p>
										<button
											onclick={() => {
												playSound('beep', isMuted);
												dalStore.hubWizardInitialStep = 5;
												dalStore.showHubModal = true;
											}}
					...
					<div
						class="bg-white rounded-[32px] p-5 border-2 {dalStore.systemMode === 'DAL'
							? 'border-[#0084CC]/10'
							: 'border-[#4B0082]/10'} shadow-sm transition-colors"
					>
						<div class="flex items-center justify-between border-b border-slate-100 pb-2 mb-3">
							<h3
								class="font-system font-black text-xs {dalStore.systemMode === 'DAL'
									? 'text-[#0084CC]'
									: 'text-[#4B0082]'} uppercase tracking-wide font-bold transition-colors"
							>
								{dalStore.systemMode === 'DAL'
									...
									>
										<div class="flex items-center gap-2">
											...
											<div class="text-xs text-left">
												<span class="font-system font-black text-slate-700 block font-bold"
													>{p.name}</span
												>
										...
										<button
											onclick={() => {
												playSound('beep', isMuted);
												handleLeaveFlight(myFlight.id, p.id);
											}}
					...
					<div class="bg-white rounded-[32px] p-5 border-2 border-[#0084CC]/10 shadow-sm space-y-4">
						<div class="border-b border-slate-100 pb-2">
							<h3
								class="font-system font-black text-xs text-[#0084CC] uppercase tracking-wide font-bold"
							>
						...
						<div
							class="bg-[#FAF8F2] p-3 rounded-2xl border border-[#E6DFC7]/50 text-xs text-slate-600 font-system italic leading-relaxed text-left"
						>
							"{myFlight.announcement ||
								'Attention passengers! Hangar seaplane prepares for flight checklist.'}"
						</div>

						<!-- AI Review segment -->
						{#if myFlight.review}
							<div
								class="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200/50 p-4 rounded-[24px] space-y-2"
							>
								<h4
									class="font-system font-black text-[#D35400] text-xs flex items-center gap-1 font-bold"
								>
									<Sparkles class="w-4 h-4 text-amber-500 fill-amber-500" />
									{dalStore.systemMode === 'DAL'
										? "Orville's Official Island Travel Review"
										: "Luna's Official Dream Review"}
								</h4>
								<p
									class="text-slate-600 text-xs italic font-sans leading-relaxed text-left font-semibold"
								>
									"{myFlight.review}"
								</p>
							</div>
						{:else}
							<div
								class="bg-[#A2D2FF]/10 border border-[#0084CC]/10 p-4 rounded-[24px] text-center"
							>
								<p class="text-sm text-slate-500 mb-2 leading-relaxed font-semibold">
									{dalStore.systemMode === 'DAL'
										...
										: 'Ask Luna to observe your dream and publish an official Dream Review brochure!'}
								</p>
								<button
				...
				<div class="md:col-span-5 space-y-4">
					...
					<div
						class="bg-[#FFFCEF] rounded-[32px] border-4 border-[#FFEAA7] p-4 shadow-sm text-[#4A4A4A]"
					>
						<div class="flex items-center gap-2 border-b border-[#FFEAA7] pb-2 mb-3">
							...
							<div>
								<h3
									class="font-system font-black text-xs {dalStore.systemMode === 'DAL'
										? 'text-[#0084CC]'
										: 'text-[#4B0082]'} uppercase leading-none font-bold"
								>
									{dalStore.systemMode === 'DAL' ? 'Smart Flight Matchmaker' : 'Dream Connector'}
								</h3>
								<span class="text-xs font-system text-slate-400 font-bold uppercase mt-0.5 block"
									>{dalStore.systemMode === 'DAL'
										? "ORVILLE'S MATCH COUNTER"
										: "LUNA'S CONNECTION COUNTER"}</span
								>
							</div>
						</div>
						...
						{#if requests.filter((r: StandbyRequest) => r.gateType === myFlight.gate).length === 0}
							<div class="text-center py-8 text-slate-400 font-system text-xs space-y-2">
								...
								<p class="text-sm leading-relaxed">
									No standby flyers are looking for Gate Category {myFlight.gate} at the moment. Keep
									radar active!
								</p>
							</div>
						{:else}
							<div class="space-y-3">
								<div
									class="bg-white/80 p-2.5 rounded-2xl border border-[#FFEAA7] text-xs text-[#4A4A4A] text-left font-semibold"
								>
									💡 <strong>{dalStore.systemMode === 'DAL' ? 'Orville' : 'Luna'}:</strong>
									"{dalStore.systemMode === 'DAL'
										? "Look! We have standby passengers looking to match your flight's category! Clear them for immediate boarding!"
								...
								{#each requests.filter((r: StandbyRequest) => r.gateType === myFlight.gate) as match, i (match.id ? match.id + '-' + i : i)}
									...
									>
											...
											<div class="text-left">
												...
												<span
													onclick={() => {
														...
														openProfileModal(
															p
																? p.userId || p.friendCode || ''
																: match.userId ||
																		match.friendCode ||
																		`SW-TEMP-${match.name}-${match.island}`
														);
													}}
												...
												<span class="text-xs text-slate-400 font-system leading-none"
													>from {match.island}</span
												>
											</div>
										</div>
										...
										<button
											onclick={() => {
												playSound('beep', isMuted);
												handleClearForTakeoff(match, myFlight.id);
											}}
					...
					<button
						onclick={() => {
							playSound('beep', isMuted);
							handleUpdateStatus(myFlight.id, 'Closed');
						}}
