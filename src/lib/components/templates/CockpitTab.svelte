...
<div class="max-w-4xl mx-auto">
  ...
  {#if !myFlight}
    <div class="bg-white rounded-[36px] border-4 border-[#0084CC]/10 shadow-[0_8px_0_0_rgba(0,132,204,0.05)] p-6 text-center space-y-6 text-left">
      ...
      <div class="max-w-xs mx-auto">
        <div class="w-20 h-20 bg-[#F0F9FF] border-2 border-[#0084CC] rounded-full flex items-center justify-center text-5xl mx-auto shadow relative transform -rotate-12">
          🛩️
          <div class="absolute -bottom-1 -right-1 bg-[#FFCC00] text-[#006094] text-[8px] font-mono font-black px-1.5 py-0.5 rounded-full border border-white uppercase">
            DAL-X
          </div>
        </div>
      </div>
      ...
      <div>
        <h2 class="text-xl font-black text-[#0084CC] text-center font-bold">Your Private DAL Seaplane Hangar</h2>
        <p class="text-xs text-slate-400 font-mono mt-1 uppercase tracking-wider font-bold text-center">
      ...
      <form onsubmit={handleHostFlight} class="max-w-xl mx-auto text-left space-y-4 border-t border-slate-100 pt-5 text-xs">
        {#if formError}
          <p class="text-xs font-bold text-red-600 flex items-center gap-1 font-mono bg-red-50 p-2.5 rounded-xl border border-red-100">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-[9px] font-mono font-black text-[#0084CC] mb-1 uppercase tracking-wider font-bold">HOST NAME</label>
            <input
              type="text"
          ...
          <div>
            <label class="block text-[9px] font-mono font-black text-[#0084CC] mb-1 uppercase tracking-wider font-bold">HOME ISLAND</label>
            <input
              type="text"
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-[9px] font-mono font-black text-[#0084CC] mb-1.5 uppercase tracking-wider font-bold">DODO CODE (5-DIGIT ALPHANUMERIC)</label>
            <input
              type="text"
              bind:value={formDodo}
              placeholder="e.g. D0D01"
              class="w-full bg-[#FAF8F2] border-[#0084CC]/30 border-2 rounded-xl px-3 py-2 text-xs font-mono font-black tracking-widest text-center uppercase outline-none focus:bg-white text-[#0084CC] font-bold"
              maxlength="5"
              required
            />
          </div>
          <div>
            <label class="block text-[9px] font-mono font-black text-[#0084CC] mb-1.5 uppercase tracking-wider font-bold">HEMISPHERE</label>
            <select
        ...
        <div>
          <label class="block text-[9px] font-mono font-black text-[#0084CC] mb-1.5 uppercase tracking-wider font-bold">GATE CATEGORY & FLIGHT PURPOSE</label>
          <div class="grid grid-cols-5 gap-2">
            {#each [1, 2, 3, 4, 5] as g}
              {@const theme = GATE_THEMES[g] || GATE_THEMES[1]}
              <button
                type="button"
              ...
              >
                <span class="text-[11px] leading-none">{theme.icon}</span>
                <span class="text-[9px] leading-none font-bold mt-0.5">Gate {g}</span>
              </button>
            {/each}
          </div>

          <div class="mt-2.5 bg-sky-50/50 p-2.5 rounded-xl border border-sky-100 flex gap-2">
            <span class="text-xl">{activeGateTheme.icon}</span>
            <div class="text-left">
              <span class="font-bold text-xs text-[#0084CC] block">{activeGateTheme.name}</span>
              <span class="text-[10px] text-slate-500 block leading-normal font-semibold">{activeGateTheme.desc}</span>
            </div>
          </div>
        </div>

        <div>
          <label class="block text-[9px] font-mono font-black text-[#0084CC] mb-1.5 uppercase tracking-wider font-bold">FLIGHT PLAN DESCRIPTION</label>
          <textarea
            bind:value={formDesc}
  ...
  {:else}
    ...
    <div class="space-y-5 text-left">
      <!-- Cockpit Card Header -->
      <div class="bg-[#006094] text-white rounded-[32px] p-5 shadow border-b-4 border-[#FFCC00] relative overflow-hidden">
        <div class="absolute right-0 top-0 opacity-10 text-9xl pointer-events-none select-none">🛩️</div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
          ...
          <div class="bg-black/20 border border-white/10 p-3 rounded-2xl text-center font-mono">
            <span class="block text-[8px] text-sky-200 font-bold uppercase">My Dodo Code</span>
            <span class="text-2xl font-black text-[#FFCC00] tracking-widest leading-none mt-0.5 block uppercase font-bold">
              {myFlight.dodoCode}
            </span>
          </div>
        </div>
        ...
        <div class="md:col-span-7 space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs">
              {#each ['Scheduled', 'Boarding', 'Departed', 'Delayed'] as FlightStatus[] as status}
                ...
                <button
                  ...
                  class="py-2 rounded-xl font-mono font-black border transition-all text-[10px] cursor-pointer font-bold {isActive ? 'bg-[#FFCC00] text-[#006094] border-2 border-[#0084CC] shadow scale-105' : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-400'}"
                >
                  {status.toUpperCase()}
                </button>
              {/each}
          ...
          <div class="bg-white rounded-[32px] p-5 border-2 border-[#0084CC]/10 shadow-sm">
            ...
            {#if myFlight.passengers.length === 0}
              <div class="py-8 text-center font-mono text-slate-400 text-xs">
                ...
                {#each myFlight.passengers as p (p.id)}
                  <div class="flex items-center justify-between bg-[#FAF8F2] border border-[#E6DFC7]/50 p-2.5 rounded-2xl">
                    ...
                    <button
                      ...
                      class="bg-slate-200 hover:bg-slate-300 text-[#4A4A4A] font-mono text-[9px] font-black px-2 py-1 rounded-full border-b border-slate-400 cursor-pointer font-bold border-none"
                    >
                      Return Home
                    </button>
                  </div>
          ...
          <div class="bg-white rounded-[32px] p-5 border-2 border-[#0084CC]/10 shadow-sm space-y-4">
            ...
            <div class="bg-[#FAF8F2] p-3 rounded-2xl border border-[#E6DFC7]/50 text-xs text-slate-600 font-mono italic leading-relaxed text-left">
            ...
            {:else}
              <div class="bg-[#A2D2FF]/10 border border-[#0084CC]/10 p-4 rounded-[24px] text-center">
                <p class="text-[11px] text-slate-500 mb-2 leading-relaxed font-semibold">
          ...
          <div class="bg-[#FFFCEF] rounded-[32px] border-4 border-[#FFEAA7] p-4 shadow-sm text-[#4A4A4A]">
            ...
            {#if requests.filter((r: StandbyRequest) => r.gateType === myFlight.gate).length === 0}
              <div class="text-center py-8 text-slate-400 font-mono text-xs space-y-2">
                ...
                <p class="text-[10px] leading-relaxed">No standby flyers are looking for Gate Category {myFlight.gate} at the moment. Keep radar active!</p>
              </div>
            {:else}
              <div class="space-y-3">
                  ...
                  <div class="bg-white p-3 rounded-2xl border-2 border-[#0084CC]/20 shadow-xs space-y-2 text-left">
                    <div class="flex items-start gap-2">
                      ...
                      <div class="text-left">
                        ...
                        <span class="text-[9px] text-slate-400 font-mono leading-none">from {match.island}</span>
                      </div>
                    </div>
