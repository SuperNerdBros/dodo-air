...
content = content.replace(
	/async function openProfileModal[\s\S]*?dalStore\.selectedFriendCode = friendCode;\n  }/,
	''
);

content = content.replace(
	/handleAddSchedule=\{handleAddSchedule\}/g,
	'handleAddSchedule={TerminalActions.addSchedule}'
);
content = content.replace(
	/handleDeleteSchedule=\{handleDeleteSchedule\}/g,
